void replay_reference_timing_shms (Int_t RunNumber = 0, Int_t MaxEvent = 0, Int_t runPeriod=0) {
  ...
  else if(runPeriod == 2019) {
    pathList.push_back("./raw-jpsi");
  }
  else {
    cerr << "ERROR! Unknown XEM Run Period.\n";
    exit;
  }
  //pathList.push_back("./raw/../raw.copiedtotape");
  //pathList.push_back("./cache");

  const char* ROOTFileNamePattern = "ROOTfiles/SHMS/TIMING/shms_reference_timing_%d_%d.root";
  ...
  //Revert back to Timing general files instead of those in standard.database
  gHcParms->AddString("g_ctp_parm_filename", "DBASE/SHMS/GEN/timing_general.param");
  gHcParms->Load(gHcParms->GetString("g_ctp_parm_filename"));
  gHcParms->Load(gHcParms->GetString("g_ctp_kinematics_filename"), RunNumber);

  //Revert back to Default calib file instead per-run calibrations. (std.dbase)
  gHcParms->AddString("g_ctp_calib_filename", "DBASE/SHMS/GEN/default_det_calib_files.param");
  gHcParms->Load(gHcParms->GetString("g_ctp_calib_filename"));


  // Load parameters for SHMS trigger configuration
  gHcParms->Load("PARAM/TRIG/tshms_refTime.param");
  ...
  // Define DEF-file for Ref. Time specific variables and histograms
  analyzer->SetOdefFile("DEF-files/SHMS/TIMING/pstackana_reftime.def");
  // Define cuts file
  analyzer->SetCutFile("DEF-files/SHMS/TIMING/CUTS/pstackana_reftime_cuts.def");  // optional
  // File to record accounting information for cuts
  //analyzer->SetSummaryFile(Form("REPORT_OUTPUT/SHMS/TIMING/summary_reference_timing_%d_%d.report", RunNumber, MaxEvent));  // optional

  // Start the actual analysis.
  analyzer->Process(run);

  // Create report file from template
  //analyzer->PrintReport("TEMPLATES/SHMS/TIMING/pstackana_reference_timing.template", Form("REPORT_OUTPUT/SHMS/TIMING/replay_refernece_timing_%d_%d.report", RunNumber, MaxEvent));  // optional
